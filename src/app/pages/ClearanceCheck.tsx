...
export default function ClearanceCheck() {
  ...
  const checkClearance = async () => {
    ...
    setLoading(true);
    try {
      // This now calls your clearance.php API
      const data = await api.checkClearance(lrn);
      
      const liabilities = [];
      
      // Map the PHP pendingRecords counts to the UI liabilities array
      if (data.pendingRecords?.equipment > 0) {
        liabilities.push({
          type: 'Equipment',
          description: `${data.pendingRecords.equipment} Unreturned Item(s)`,
          status: 'pending',
          date: new Date().toISOString(),
        });
      }
      if (data.pendingRecords?.guidance > 0) {
        liabilities.push({
          type: 'Guidance',
          description: `${data.pendingRecords.guidance} Pending Case(s)`,
          status: 'pending',
          date: new Date().toISOString(),
        });
      }
      if (data.pendingRecords?.facilities > 0) {
        liabilities.push({
          type: 'Facilities',
          description: `${data.pendingRecords.facilities} Damage Report(s)`,
          status: 'pending',
          date: new Date().toISOString(),
        });
      }

      // Update the state with REAL data from the database
      setClearanceData({
        student: {
      ...
      if (data.cleared) {
        toast.success('Student is cleared!');
      } else {
        toast.warning('Student has pending liabilities');
      }

    } catch (error: any) {
      toast.error(error.message || 'Failed to check clearance. Ensure the LRN is correct.');
